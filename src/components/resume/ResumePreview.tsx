...
export function ResumePreview({ data }: ResumePreviewProps) {
  ...
  return (
    <div className="resume-paper p-8 font-sans text-sm leading-relaxed overflow-y-auto" style={{ fontFamily: 'Georgia, serif' }}>
      {/* Header */}
      <header className="text-center border-b-2 border-slate-800 pb-4 mb-4">
        <h1 className="text-2xl font-bold text-slate-900 tracking-wide" style={{ fontFamily: 'Georgia, serif' }}>
      ...
      {!personalInfo.fullName && !summary && experiences.length === 0 && education.length === 0 && skills.length === 0 && (
        <div className="flex items-center justify-center h-full text-slate-400 text-center">
          <div>
            ...
            <p className="text-xs">Fill out the form on the left to see your resume here</p>
          </div>
        </div>
      )}
    </div>
